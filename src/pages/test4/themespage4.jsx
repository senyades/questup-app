...
function Themespage_4({ testId }) {
  ...
  useEffect(() => {
    const fetchDataAndProcess = async () => {
      ...
      if(test_data[0][3].thema == 1 && thisthema != 0) {
        newthisThema(0);
      }
  ...
  const isActive = (themeId, testId) => {
    ...
    const isThemeActive = parseInt(test_data[0][3]?.thema) === parseInt(themeId);
  ...
  const isView = (themeId) => {
    ...
    console.log(test_data[0][3].thema)
    // Создаем массив с числами от 1 до значения темы
    const themeNumbers = Array.from({ length: parseInt(test_data[0][3]?.thema) }, (_, index) => index);
     ...
     <Header_Test GoLink={'/tests'}/>
      <div className="flex w-2/6 flex-col items-center h-full  gap-8 justify-center ">
          <div className="flex flex-col items-center content-center  w-full gap-1">
                ...
                <span className="text-2xl font-medium ">Создание контента</span>
