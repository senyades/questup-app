 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      {fireworkstrue === true && <Firework diamonds={inventoryTestData.diamonds} exp={inventoryTestData.exp} page={6} text={"Вы ответили на вопрос"}/>}
      ...
      <span className='text-lg text-gray-400 font-normal text-center'>Выберите 1 вариант ответа. За каждую попытку снимается 2 очка</span>
        ...
        <span className='text-2xl font-semibold text-center'>Понятие «цифровая грамотность» впервые было введено:</span>
        <div className='flex flex-col gap-2'>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'first' ? styles.false : ''}`} onClick={() => handleCheck('first')}>Правительством России в августе 2017 года
        </div>
        <div className={`${styles.variantblock}  ${selectedOption ? styles.check : ''}  ${selectedOption === 'second' ? styles.true : ''}`} onClick={() => handleCheck('second')}>Пол Гилстер, американский писатель и журналист в 2017 году
        </div>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'third' ? styles.false : ''}`} onClick={() => handleCheck('third')}>Пол Гилстер, американский писатель и журналист в 1997 году
        </div>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'fourth' ? styles.false : ''}`} onClick={() => handleCheck('fourth')}>Национальная программа «Цифровая экономика РФ», 2017 год
        </div>
        </div>
      </div>
