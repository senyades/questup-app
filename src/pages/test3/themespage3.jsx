...
function Themespage_3({ testId }) {
  ...
  const isActive = (themeId, testId) => {
    ...
    const isThemeActive = parseInt(test_data[0][2]?.thema) === parseInt(themeId);
  ...
  const isView = (themeId) => {
    ...
    console.log(test_data[0][2].thema)
    // Создаем массив с числами от 1 до значения темы
    const themeNumbers = Array.from({ length: parseInt(test_data[0][2]?.thema) }, (_, index) => index);
     ...
     <Header_Test GoLink={'/tests'}/>
      <div className="flex w-2/6 flex-col items-center h-full  gap-8 justify-center ">
          <div className="flex flex-col items-center content-center  w-full gap-1">
                ...
                <span className="text-2xl font-medium ">Информационная грамотность</span>
