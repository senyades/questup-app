...
function Themespage_1({ testId }) {
  ...
  const isActive = (themeId, testId) => {
    ...
    const isThemeActive = parseInt(test_data[0][0]?.thema) === parseInt(themeId);
  ...
  const isView = (themeId) => {
    ...
    console.log(test_data[0][0].thema)
    // Создаем массив с числами от 1 до значения темы
    const themeNumbers = Array.from({ length: parseInt(test_data[0][0]?.thema) }, (_, index) => index);
     ...
     <Header_Test GoLink={'/tests'}/>
      <div className="flex w-2/6 flex-col items-center h-full  gap-8 justify-center ">
          <div className="flex flex-col items-center content-center  w-full gap-1">
                <span className="text-2xl font-medium ">Основы цифровой грамотности</span>
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <NavLink to={`/tests/play/1/testpage`} onClick={()=> {newthisThema(2)}} className={`${styles.themeblock} ${isActive(2, testId) && styles.active} ${isView(2) && styles.view}`}>Компоненты цифровой грамотности
            <div className={`${styles.infotext} flex flex-row gap-2 `}>
                        <div className="flex flex-row items-center gap-2 h-7 px-2 w border-slate-200 bg-slate-50 rounded-lg text-sm text-gray-400 text-center font-normal">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                          <g clip-path="url(#clip0_251_188)">
            ...
            <NavLink to={`/tests/play/1/testpage`} onClick={()=> {newthisThema(3)}} className={`${styles.themeblock} ${isActive(3, testId) && styles.active} ${isView(3) && styles.view}`}>Цифровые компетенции
            <div className={`${styles.infotext} flex flex-row gap-2 `}>
                        <div className="flex flex-row items-center gap-2 h-7 px-2 w border-slate-200 bg-slate-50 rounded-lg text-sm text-gray-400 text-center font-normal">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                          <g clip-path="url(#clip0_251_188)">
