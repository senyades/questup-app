 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      {fireworkstrue === true && <Firework diamonds={inventoryTestData.diamonds} exp={inventoryTestData.exp} page={4} text={"Вы ответили на вопрос"}/>}
      ...
      <span className='text-lg text-gray-400 font-normal text-center'>Выберите 1 вариант ответа. За каждую попытку снимается 2 очка</span>
        ...
        <span className='text-2xl font-semibold text-center'>Компетенция - это</span>
        <div className='flex flex-col gap-2'>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'first' ? styles.false : ''}`} onClick={() => handleCheck('first')}>знания, умения, навыки</div>
        <div className={`${styles.variantblock}  ${selectedOption ? styles.check : ''}  ${selectedOption === 'second' ? styles.true : ''}`} onClick={() => handleCheck('second')}>знания, умения, навыки, модели поведения и личностные характеристики</div>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'third' ? styles.false : ''}`} onClick={() => handleCheck('third')}>Soft и Hard Skills</div>
        </div>
      </div>
