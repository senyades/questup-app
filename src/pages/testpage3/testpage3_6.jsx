 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      {fireworkstrue === true && <Firework diamonds={inventoryTestData.diamonds} exp={inventoryTestData.exp} page={7} text={"Вы ответили на вопрос"}/>}
      ...
      <span className='text-lg text-gray-400 font-normal text-center'>Выберите 1 вариант ответа. За каждую попытку снимается 2 очка</span>
        ...
        <span className='text-2xl font-semibold text-center'>Soft skills - это</span>
        <div className='flex flex-col gap-2'>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'first' ? styles.false : ''}`} onClick={() => handleCheck('first')}>профессиональные навыки, которые необходимы для выполнения конкретной работы</div>
        <div className={`${styles.variantblock}  ${selectedOption ? styles.check : ''}  ${selectedOption === 'second' ? styles.true : ''}`} onClick={() => handleCheck('second')}>личные качества, которые зависят от характера человека и приобретаются с жизненным опытом</div>
        </div>
      </div>
