  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      <span className='text-lg text-gray-400 font-normal text-center'>Выберите 1 вариант ответа. За каждую попытку снимается 2 очка</span>
        ...
        <span className='text-2xl font-semibold text-center'>"Облачные технологии" - что это?</span>
        <div className='flex flex-col gap-2'>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'first' ? styles.false : ''}`} onClick={() => handleCheck('first')}>Технологии для совместной работы. Предварительно приложения нужно установить на свой компьютер</div>
        <div className={`${styles.variantblock}  ${selectedOption ? styles.check : ''}  ${selectedOption === 'second' ? styles.true : ''}`} onClick={() => handleCheck('second')}>Технологии, благодаря которым пользователи получают доступ к компьютерным ресурсам в онлайне</div>
        <div className={`${styles.variantblock} ${selectedOption ? styles.check : ''} ${selectedOption === 'third' ? styles.false : ''}`} onClick={() => handleCheck('third')}>Это облачные хранилища для совместной работы с файлами</div>
        </div>
      </div>
