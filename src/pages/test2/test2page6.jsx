 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      {fireworkstrue === true && <Firework diamonds={inventoryTestData.diamonds} exp={inventoryTestData.exp} page={7} text={"Вы ответили на тест"}/>}
      ...
      <span className='text-lg text-gray-400 font-normal'>Выберите варианты ответа. За каждый неверный ответ снимаются баллы</span>
            ...
            <span className='text-2xl font-semibold'>Какие особенности Google Диска позволяют повысить эффективность работы?</span>
            <div className='flex flex-col gap-2'>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedFirst==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('first'); setSelectedFirst(true)}}>Удобный интерфейс и простота использования</div>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedSecond==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('second'); setSelectedSecond(true)}}>Интеграция с другими сервисами Google</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedThird==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('third'); setSelectedThird(true)}}>Возможность работы с различными типами файлов</div>
          </div>
          </div>
