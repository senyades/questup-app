...
const Test5page3 = () => {
 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  const handleCheck = (option) => {
    ...
    if (option=="five" || option=="six" || option=="seven" )
    {
      setRight(right+1)
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      ...
      <span className='text-lg text-gray-400  font-normal'>Выберите 3 варианта ответа. За каждый неверный ответ снимаются баллы</span>
            ...
            <span className='text-2xl font-semibold  flex'>Отметьте ресурсы, где можно найти онлайн курсы:
</span>
            <div className='flex flex-col gap-2 w-full'>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedFirst==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('first'); setSelectedFirst(true)}}>studentlibrary.ru</div>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedSecond==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('second'); setSelectedSecond(true)}}>urait.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedThird==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('third'); setSelectedThird(true)}}>e.lanbook.com</div>
            ...
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedFive==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('five'); setSelectedFive(true)}}>cyberleninka.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedSix==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('six'); setSelectedSix(true) }}>intuit.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedSeven==true && !truepoint ? styles.checkVar : ''} ${truepoint  ? styles.true : ''}`} onClick={() => {handleCheck('seven'); setSelectedSeven(true)}}>stepik.org</div>
          </div>
          </div>
      ...
      {nextpoint === false &&   <div className={` w-48`}> <Button onClick={() => {CheckAnswer();setTruePoint(true);}}>Ответить</Button> </div>}
        ...
        {test_data[0][4].thema != 1 &&
      <div className="flex flex-row w-full items-center justify-center p-4 border-t border-gray-200 bg-[#ffffff]">
        <NavLink to={"/tests/play/5"} className="w-40"> <Button onClick={()=> {handlePageChange(0);}}>Завершить</Button></NavLink>
    </div>
      }
    </div>
  );
