...
function Themespage_5({ testId }) {
  ...
  const isActive = (themeId, testId) => {
    ...
    const isThemeActive = parseInt(test_data[0][4]?.thema) === parseInt(themeId);
  ...
  const isView = (themeId) => {
    ...
    console.log(test_data[0][4].thema)
    // Создаем массив с числами от 1 до значения темы
    const themeNumbers = Array.from({ length: parseInt(test_data[0][4]?.thema) }, (_, index) => index);
