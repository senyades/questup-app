 ...
 const [inventoryTestData, setInventoryTestData] = useState({
  ...
  return (
    <div className="flex flex-col h-full w-full items-center bg-[#f9fafb]">
      <Header_Test GoLink={'/tests/play/5'}/>
      <div className="flex w-full flex-col items-center h-full overflow-y-scroll gap-8 justify-center ">

      {fireworkstrue === true && <Firework diamonds={inventoryTestData.diamonds} exp={inventoryTestData.exp} page={2} text={"Вы ответили на тест"}/>}
      ...
      <span className='text-lg text-gray-400  font-normal'>Выберите 3 варианта ответа. За каждый неверный ответ снимаются баллы</span>
            ...
            <span className='text-2xl font-semibold w-2/6 flex'>Отметьте учебные библиотеки из предложенного списка (где можно найти учебную литературу (учебники, пособия и т. .п)):</span>
            <div className='flex flex-col gap-2 w-2/6'>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedFirst==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('first'); setSelectedFirst(true)}}>studentlibrary.ru</div>
            <div className={`${styles.variantblock}  ${nextpoint === false ? styles.check : ''} ${selectedSecond==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('second'); setSelectedSecond(true)}}>urait.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedThird==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.true : ''}`} onClick={() => {handleCheck('third'); setSelectedThird(true)}}>e.lanbook.com</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedFourth==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('fourth'); setSelectedFourth(true)}}>elibrary.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedFive==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('five'); setSelectedFive(true)}}>cyberleninka.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedSix==true && !truepoint ? styles.checkVar : ''} ${truepoint ? styles.false : ''}`} onClick={() => {handleCheck('six'); setSelectedSix(true) }}>intuit.ru</div>
            <div className={`${styles.variantblock} ${nextpoint === false ? styles.check : ''} ${selectedSeven==true && !truepoint ? styles.checkVar : ''} ${truepoint  ? styles.false : ''}`} onClick={() => {handleCheck('seven'); setSelectedSeven(true)}}>stepik.org</div>
          </div>
          </div>
